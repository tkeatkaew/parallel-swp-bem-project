#!/usr/bin/env bash
################################################################################
# benchmark_openblas_v2.sh - OpenBLAS Benchmark with Pure Sequential Baseline
#
# VERSION 2: True Sequential Baseline
# - Method 0: Sequential multiply + Gauss-Jordan inversion (pure sequential)
# - Methods 1-3: OpenBLAS multiply + LAPACK inversion (parallel)
#
# This ensures Method 0 is a TRUE baseline with no parallel optimizations.
#
# Usage: ./benchmark_openblas_v2.sh [NUM_THREADS] [ARG1 ARG2 ARG3]
#
# Example:
#   ./benchmark_openblas_v2.sh 6 1.0 100.0 0.001
#
# Author: Optimized for BEM Flow Path Analysis
# Date: 2025-11-21 (Version 2)
################################################################################
...
setup_environment() {
    ...
    # OpenBLAS threading (for methods 1-3)
...
run_test() {
    ...
    # CRITICAL: Set inversion method based on multiply method
    # Method 0 = sequential multiply + sequential inversion
    # Method 1-3 = OpenBLAS multiply + LAPACK inversion
    local inversion_method
    if [ "$method" -eq 0 ]; then
        inversion_method=1  # 1 = Sequential (Gauss-Jordan)
    else
        inversion_method=0  # 0 = Parallel (LAPACK)
    fi
...
main() {
    print_header "OpenBLAS Benchmark Suite v2 - Pure Sequential Baseline"
    ...
    # Results table
    printf "%-45s %12s %12s %12s %12s %12s %15s\n" \
        "Method" "Wall(s)" "Mult(s)" "Inv(s)" "Total(s)" "Speedup" "Inv Type"
    printf "%-45s %12s %12s %12s %12s %12s %15s\n" \
        "------" "-------" "-------" "------" "--------" "-------" "--------"
    ...
    tail -n +2 "$OUTPUT_DIR/results.csv" | while IFS='|' read -r method name area dgemm inv comp wall gflops inv_method; do
        ...
        printf "%-45s %12.2f %12.2f %12.2f %12.2f %12s %15s\n" \
            "$name" "$wall" "$dgemm" "$inv" "$comp" "$speedup" "$inv_type"
    done
    ...
    ts "Key Takeaway:"
    ts "  Method 0 speedup now reflects TRUE parallel performance gain"
