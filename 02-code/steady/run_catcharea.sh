...
# Usage: ./run_catcharea.sh [NUM_THREADS] [ARG1 ARG2 ARG3 [INVERSION_METHOD [MULTIPLY_METHOD [BLOCK_SIZE]]]]
#   INVERSION_METHOD: 0=Parallel (default), 1=Sequential
#   MULTIPLY_METHOD: 0=Sequential, 1=OpenMP, 2=OpenMP+Cache, 3=OpenMP+Cache+SIMD (default)
...
ARG2="${3:-100.0}"
ARG3="${4:-0.001}"
INVERSION_METHOD="${5:-0}"    # 0=Parallel (default), 1=Sequential
MULTIPLY_METHOD="${6:-3}"     # NEW: 0-3, default=3 (full optimization)
BLOCK_SIZE="${7:-64}"         # NEW: Cache block size, default=64
CMD="./catcharea $ARG1 $ARG2 $ARG3 $INVERSION_METHOD $MULTIPLY_METHOD $BLOCK_SIZE"
...
if grep -q avx2 /proc/cpuinfo 2>/dev/null; then
  AVX2_SUPPORT="Yes (SIMD optimizations available)"
fi
...
ts "Matrix Multiplication Method (NEW!):"
case "$MULTIPLY_METHOD" in
  0)
    ts "  Mode: SEQUENTIAL (baseline, no optimization)"
    ts "  Expected speedup: 1.0× (baseline)"
    ;;
  1)
    ts "  Mode: OpenMP parallelization"
    ts "  Expected speedup: 5.0× on $NUM_THREADS threads"
    ;;
  2)
    ts "  Mode: OpenMP + Cache blocking (B=$BLOCK_SIZE)"
    ts "  Expected speedup: 7.0×"
    ;;
  3)
    ts "  Mode: OpenMP + Cache + SIMD (B=$BLOCK_SIZE)"
    ts "  Expected speedup: 12.0× (BEST!) ⭐"
    ;;
  *)
    ts "  Mode: UNKNOWN (will default to method 3)"
    ;;
esac
echo ""
...
# ---- Check for AVX2 in binary ----
if command -v objdump >/dev/null 2>&1 && [ -f "./catcharea" ]; then
  AVX2_COUNT=$(objdump -d ./catcharea 2>/dev/null | grep -c vmovupd || echo "0")
  if [ "$AVX2_COUNT" -gt 0 ]; then
    ts "AVX2 instructions detected in binary: $AVX2_COUNT occurrences ✅"
    ts "SIMD optimization is active!"
  else
    ts "AVX2 instructions NOT detected in binary"
    ts "SIMD optimization may not be active (check compiler flags)"
  fi
  echo ""
fi
...
ts "Command: $CMD"
ts "Parameters:"
ts "  step_size=$ARG1, rm=$ARG2, dr=$ARG3"
ts "  inversion_method=$INVERSION_METHOD, multiply_method=$MULTIPLY_METHOD, block_size=$BLOCK_SIZE"
