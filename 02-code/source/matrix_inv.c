...
// Fallback prototype
static lapack_int mat_inv_gauss_fallback(double *A, unsigned n);
...
lapack_int mat_inv(double *A, unsigned n)
{
    ...
    if (ret != 0) {
        ...
        printf(">> Falling back to Gauss-Jordan inversion (sequential)...\n");
        return mat_inv_gauss_fallback(A, n);
    }
    ...
    // ✅ ใหม่: แจ้ง global performance summary ด้วย
    //update_inversion_time(duration, n);
...
/*----------------------------------------------------------------------------------*/
/* Fallback Gauss-Jordan inversion (sequential) */
/*--------------------------------------------------------------------
 * Fallback: Gauss-Jordan matrix inversion (sequential, row-major)
 * A : column-major n x n matrix (in-place, overwritten by A^{-1})
 *--------------------------------------------------------------------*/
static lapack_int mat_inv_gauss_fallback(double *A, unsigned n)
{
    unsigned i, j, k;
    ...
    unsigned max_row;
    double *aug = NULL;
    unsigned cols = 2 * n;  // [A | I]

    printf("=== Gauss-Jordan Fallback Inversion ===\n");
    printf("Matrix size: %u x %u (sequential)\n", n, n);

    if (A == NULL) {
        fprintf(stderr, "Gauss-Jordan fallback: A is NULL (n=%u)\n", n);
        return -1;
    }

    aug = (double *)malloc((size_t)n * cols * sizeof(double));
    if (!aug) {
        fprintf(stderr,
                "Gauss-Jordan fallback: malloc failed for %zu bytes\n",
    ...
    for (i = 0; i < n; ++i) {
        for (j = 0; j < n; ++j) {
            /* A is column-major: element(i,j) = A[j*n + i] */
            aug[i*cols + j] = A[j*n + i];
        }
        for (j = 0; j < n; ++j) {
            aug[i*cols + (n + j)] = (i == j) ? 1.0 : 0.0;
        }
    }
    ...
    for (i = 0; i < n; ++i) {
        ...
        max_row = i;
        max_val = fabs(aug[i*cols + i]);
        for (k = i + 1; k < n; ++k) {
            double v = fabs(aug[k*cols + i]);
            if (v > max_val) {
                max_val = v;
                max_row = k;
            }
        ...
        if (max_val == 0.0) {
            fprintf(stderr,
                    "Gauss-Jordan fallback: singular matrix at column %u\n", i);
            free(aug);
            return -3;
        }
        ...
        if (max_row != i) {
            for (j = 0; j < cols; ++j) {
                ...
                aug[max_row*cols + j] = temp;
            }
        }

        /* Normalize pivot row */
        {
            double pivot = aug[i*cols + i];
            for (j = 0; j < cols; ++j) {
                aug[i*cols + j] /= pivot;
            }
        }
    ...
    /* Copy inverse (right half) back into A, as column-major */
    for (i = 0; i < n; ++i) {
        for (j = 0; j < n; ++j) {
            double val = aug[i*cols + (n + j)];
    ...
    printf("Gauss-Jordan fallback: inversion complete (sequential).\n");
    return 0;
}
